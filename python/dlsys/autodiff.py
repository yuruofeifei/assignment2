...
class MatMulOp(Op):
    ...
    def infer_shape(self, node, input_shapes):
        """TODO: Your code here""" 
        return (input_shapes[0][1 if node.matmul_attr_trans_A else 0], input_shapes[1][0 if node.matmul_attr_trans_B else 1])
    ...
    def infer_shape(self, feed_shapes):
        ...
        for node in self.topo_order:
            if node not in feed_shapes:
                shapes = map(lambda x: node_to_shape_map[x], node.inputs)
                node_to_shape_map[node] = node.op.infer_shape(node, shapes)
            else:
